...
Boston Housing Dataset in MASS package
...
train.label  <- Boston$medv
train.matrix <- Boston %>% select(-medv) %>% as.matrix
...
# With no option
```{r}
plot.umap <- function(.umap, label = NULL, color = label, title = "") {
  
  mapping.umap <- data.frame(
    id     = 1:NROW(result.umap),
    dim1  = result.umap[, 1],
    dim2  = result.umap[, 2])
...
```{r}
result.umap <- train.matrix %>% 
  uwot::umap()

ggp.umap <- result.umap %>% 
  plot.umap(
    label = (train.label),
